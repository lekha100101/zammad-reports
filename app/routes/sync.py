...
def ensure_sync_access(request: Request, db: Session, x_sync_token: str | None = None):
...
@router.post("/users")
def sync_users(request: Request, db: Session = Depends(get_db), x_sync_token: str | None = Header(default=None)):
    ensure_sync_access(request, db, x_sync_token)
    return get_sync(db).sync_users()


@router.post("/groups")
def sync_groups(request: Request, db: Session = Depends(get_db), x_sync_token: str | None = Header(default=None)):
    ensure_sync_access(request, db, x_sync_token)
    return get_sync(db).sync_groups()


@router.post("/organizations")
def sync_organizations(request: Request, db: Session = Depends(get_db), x_sync_token: str | None = Header(default=None)):
    ensure_sync_access(request, db, x_sync_token)
    return get_sync(db).sync_organizations()


@router.post("/ticket-states")
def sync_states(request: Request, db: Session = Depends(get_db), x_sync_token: str | None = Header(default=None)):
    ensure_sync_access(request, db, x_sync_token)
    return get_sync(db).sync_ticket_states()


@router.post("/tickets")
def sync_tickets(request: Request, db: Session = Depends(get_db), x_sync_token: str | None = Header(default=None)):
    ensure_sync_access(request, db, x_sync_token)
    return get_sync(db).sync_tickets()


@router.post("/ticket-history")
def sync_ticket_history(
    request: Request,
    ...
    ensure_sync_access(request, db, x_sync_token)
    return get_sync(db).sync_ticket_history(ticket_id=ticket_id)
